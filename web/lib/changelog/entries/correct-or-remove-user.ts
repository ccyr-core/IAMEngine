...
export const entry: ChangelogEntry = {
  ...
  items: [
    ...
    "Remove user is only offered for 30 days after the onboard; after that, offboard the user instead. Correct user is no longer offered once the user has been removed. Google keeps a deleted user restorable for 20 days (it can't be purged sooner); AD and Microsoft 365 deletes are permanent",
    "Users synced from AD are changed or removed in AD, and directory sync carries it to Microsoft 365. These steps only go to runners on 1.127.0 or later",
  ],
};
