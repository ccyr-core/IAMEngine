...
function stubDb(opts: { action?: string; ageDays?: number; jobs?: J[]; payload?: Record<string, unknown>; client?: { backbone: string | null; identity: unknown }; busyInTx?: string; agents?: Array<{ name: string; semver: string | null; capabilities?: unknown }> } = {}) {
  ...
  const db = {
    ...
    agent: { findMany: async () => opts.agents ?? [{ name: "dc01", semver: "1.136.0" }] },
...
test("L2: runners older than 1.136.0 (or not reporting a version) are withheld the correct/remove keys", () => {
  ...
  assert.deepEqual(userAdhocVersionExclusions("1.136.0"), []);
...
test("L2: a 'skipped' correct/remove result is recorded as failed (not done); other keys are untouched", () => {
  ...
  assert.match(r.error ?? "", /runner 1.136.0 or later/);
...
test("N3: an AD correct/remove is refused when every runner of the client is older than 1.136.0", async () => {
  const { db, created } = stubDb({ agents: [{ name: "dc01", semver: "1.126.4" }, { name: "dc02", semver: null }] });
  const r = await dispatchUserAdhoc(db, "case", "remove", "t");
  assert.equal(r.ok, false);
  assert.match(String((r as { error: string }).error), /updated to 1.136.0 or later .* dc01 \(1\.126\.4\), dc02 \(version unknown\)/);
  assert.equal(created.length, 0);
  const ok = stubDb({ agents: [{ name: "dc01", semver: "1.126.4" }, { name: "dc02", semver: "1.136.0" }] });
...
test("N3 (final review): refused when the client has no enabled runner, or none that can run AD", async () => {
  ...
  const noAd = stubDb({ agents: [{ name: "fs01", semver: "1.136.0", capabilities: ["directory-sync"] }] });
  ...
  const ok = stubDb({ agents: [{ name: "fs01", semver: "1.136.0", capabilities: ["directory-sync"] }, { name: "dc01", semver: "1.136.0", capabilities: ["active-directory"] }] });
  assert.equal((await dispatchUserAdhoc(ok.db, "case", "remove", "t")).ok, true);
});
