...
function stubDb(opts: { action?: string; ageDays?: number; jobs?: J[]; payload?: Record<string, unknown>; client?: { backbone: string | null; identity: unknown }; busyInTx?: string; agents?: Array<{ name: string; semver: string | null; capabilities?: unknown }> } = {}) {
  ...
  const db = {
    ...
    agent: { findMany: async () => opts.agents ?? [{ name: "dc01", semver: "1.130.0" }] },
...
test("L2: runners older than 1.130.0 (or not reporting a version) are withheld the correct/remove keys", () => {
  ...
  assert.deepEqual(userAdhocVersionExclusions("1.130.0"), []);
...
test("N3: an AD correct/remove is refused when every runner of the client is older than 1.130.0", async () => {
...
test("N3 (final review): refused when the client has no enabled runner, or none that can run AD", async () => {
  ...
  const noAd = stubDb({ agents: [{ name: "fs01", semver: "1.130.0", capabilities: ["directory-sync"] }] });
  ...
  const ok = stubDb({ agents: [{ name: "fs01", semver: "1.130.0", capabilities: ["directory-sync"] }, { name: "dc01", semver: "1.130.0", capabilities: ["active-directory"] }] });
  assert.equal((await dispatchUserAdhoc(ok.db, "case", "remove", "t")).ok, true);
});
