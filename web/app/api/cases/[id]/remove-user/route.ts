...
// `confirm` must repeat the user's email/UPN exactly: a typed guard against removing the wrong case.
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const c = await db.caseRequest.findUnique({ where: { id: params.id }, select: { payload: true } });
  const p = (c?.payload ?? {}) as Record<string, unknown>;
  const upn = String(p.userPrincipalName ?? p.workEmail ?? "").trim();
  ...
  if (typeof body.confirm !== "string" || body.confirm.trim().toLowerCase() !== upn.toLowerCase()) {
    return NextResponse.json({ error: `type the user's email (${upn}) exactly to confirm` }, { status: 422 });
  }
